-- ============================================================
-- FIX: subject_ids per student_term + term tagging
-- Run this in Supabase SQL Editor.
-- Safe to run on live production; all idempotent.
-- ============================================================

-- STEP 1 ─ Add subject_ids column to student_terms
-- Stores exactly which subjects each term was created with (set at creation time).
...
-- Matches enrolled subjects to the student_term via term_id + level.
...
-- STEP 3 ─ Tag global terms by name (if not already)
...
-- STEP 4 ─ Tag the active term as 'may' if still untagged (May 2026 is current)
UPDATE public.terms
SET term_type = 'may'
...
-- STEP 6 ─ Verify: check no student_terms still have empty subject_ids
SELECT
